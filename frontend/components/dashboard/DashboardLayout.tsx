...
function WishperMELogo({ collapsed }: { collapsed: boolean }) {
  return (
    <div className={cn('flex items-center gap-2.5 px-4 h-14 border-b border-border flex-shrink-0', collapsed && 'justify-center px-3')}>
      ...
      {!collapsed && (
        <div className="min-w-0">
          ...
          <div className="text-[10px] text-muted-foreground leading-tight">Telegram Autopilot</div>
        </div>
      )}
    </div>
  );
        ...
        {!collapsed && (
          <>
            <div className="flex-1 min-w-0 text-left">
              <div className={cn('text-xs font-medium truncate leading-tight', isIncomplete && 'text-orange-400')}>
                {selected?.display_name || selected?.name || 'Incomplete Creator'}
              </div>
              {selected?.telegram_phone && (
                <div className="text-[10px] text-muted-foreground truncate leading-tight">{selected.telegram_phone}</div>
              )}
            </div>
          ...
          <div className="absolute top-full left-3 right-3 mt-1 bg-popover border border-border rounded-lg shadow-xl z-50 overflow-hidden">
            {creators.map(c => (
              <button key={c.id} onClick={() => { switchCreator(c.id); setOpen(false); }}
                className={cn('w-full flex items-center gap-2.5 px-3 py-2.5 text-sm hover:bg-accent transition-colors', c.id === selected?.id && 'bg-accent')}>
                ...
                <div className="flex-1 min-w-0 text-left">
                  <div className="font-medium truncate text-sm">{c.display_name || c.name || 'Unnamed'}</div>
                  <div className="text-xs text-muted-foreground truncate">
                    {c.telegram_phone || <span className="text-orange-400">No phone connected</span>}
                  </div>
                </div>
...
function NavItem({
      ...
      <div>
        <button
          ...
          className={cn(
            ...
            (isActive || childActive) ? 'bg-primary/10 text-primary' : 'text-muted-foreground hover:bg-accent hover:text-foreground'
          )}
        >
          {Icon && <Icon className="flex-shrink-0 h-4 w-4" />}
  ...
  return (
    <Link href={item.href} onClick={onNav}>
      <div className={cn(
        'flex items-center rounded-lg px-2.5 py-2 text-sm font-medium transition-all',
        collapsed ? 'justify-center' : 'gap-3',
        isActive
          ? 'bg-primary text-primary-foreground shadow-sm'
          : childActive
          ? 'bg-primary/10 text-primary'
          : cn('text-muted-foreground hover:bg-accent hover:text-foreground', item.accent),
      )}>
        {Icon && <Icon className={cn('flex-shrink-0 h-4 w-4', !isActive && item.accent)} />}
...
function DashboardInner({ children }: DashboardLayoutProps) {
      ...
      {!isMobile && (
        <div className="p-3">
          <button onClick={() => setCollapsed(c => !c)}
            className={cn('w-full flex items-center gap-2 rounded-lg px-2.5 py-2 text-xs text-muted-foreground hover:bg-accent hover:text-foreground transition-colors', collapsed ? 'justify-center' : '')}>
  ...
  return (
    <div className="flex min-h-screen bg-background text-foreground">
      {!mobile && (
        <aside className={cn('flex flex-col flex-shrink-0 bg-card border-r border-border sticky top-0 h-screen transition-all duration-200 overflow-hidden', collapsed ? 'w-[56px]' : 'w-[232px]')}>
          <SidebarContent />
        </aside>
      )}
      {mobile && (
        <div className="fixed top-0 left-0 right-0 z-50 h-14 flex items-center gap-3 px-4 bg-card border-b border-border">
          <button onClick={() => setDrawer(d => !d)} className="p-1.5 rounded-md hover:bg-accent transition-colors">
      ...
      {mobile && drawer && (
        <>
          <div onClick={() => setDrawer(false)} className="fixed inset-0 bg-black/50 z-50" />
          <aside className="fixed top-0 left-0 bottom-0 z-[60] w-[232px] flex flex-col bg-card border-r border-border slide-in-from-left">
